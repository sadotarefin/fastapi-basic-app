...
from app.exceptions.exception_handler import ConflictException, NotFoundException
...
class UserService:
    ...
    @staticmethod
    def get_user(session: Session, user_id: int) -> UserPublic:
        user = UserCrud.get(session, user_id)
        if not user:
